...
namespace ETHotfix
{
    ...
    public class UIManagerComponent: Component
    {
        ...
        public void Awake()
        {
            ...
            Log.Info("Now is: editor mode");
#else
             Log.Info("Now is: AB包 mode");
            //加载AB包
#endif
            //Groot的参数初始化
        ...
        {
            _DicAllUIForms.TryGetValue(uiType, out BaseUIForms baseUIForms);
        ...
        {
            //检查是否已经存在显示中
            _DicCurrentShowUIForms.TryGetValue(uiType, out BaseUIForms baseUiForms);
            if (baseUiForms != null)
            {
                return;
            }
            ...
            _DicAllUIForms.TryGetValue(uiType, out BaseUIForms allDictionaryForms);
        ...
        {
            _DicCurrentShowUIForms.TryGetValue(uiType, out BaseUIForms baseUIForm);
            ...
            _DicAllUIForms.TryGetValue(uiType, out BaseUIForms baseUIFormFormAll);
        ...
        private void ExitReverseChangeForms()
        {
            if (_StaCurrentUIForms.Count >= 2)
            {
                ...
                topUIForm.Hiding();
                //因为Hiding的原因，会导致栈里的count提前减去。在这里判断一下防止emptyStack异常，以后有好的思路再优化！！！！！！！！！
        ...
        {
            if (this._StaCurrentUIForms == null || this._StaCurrentUIForms.Count <= 0)
                return;
        ...
        private BaseUIForms LoadUIFormFromAllUIFormsCatch(Type uiType)
        {
            ...
            _DicAllUIForms.TryGetValue(uiType, out BaseUIForms baseUIFormsResult);

            return baseUIFormsResult ?? (baseUIFormsResult = this.CreateUiForms(uiType));
        }
        ...
        private BaseUIForms CreateUiForms(Type uiType)
        {
            ...
            if (baseUIForms == null)
            {
                Log.Error($"错误!BaseUIForms为空：{uiType.Name},请检查相关配置");
                return null;
            }
