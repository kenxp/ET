...
//#else
            if (UIPackage.GetByName(pakName) == null)
            {
                ...
                Log.Error($"初始化Ui bundleName:{bundleName}");
                Game.Scene.GetComponent<ResourcesComponent>().LoadOneBundle(bundleName);
                AssetBundle bundle = Game.Scene.GetComponent<ResourcesComponent>().bundles[bundleName].AssetBundle;
                //赋值给FairyGUi。
