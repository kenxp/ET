...
#endif
            Log.Info("uipackages遍历");
            var uiPackages = UIPackage.GetPackages();
            foreach (var pak in uiPackages)
            {
                Log.Info("pak.name" + pak.name);
                Log.Info("pak.id" + pak.id);
               }
                Log.Info("uipackages遍历结束");
                //Groot的参数初始化
